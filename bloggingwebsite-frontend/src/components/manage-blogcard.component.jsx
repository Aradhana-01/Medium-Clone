            ...
            <div className="flex gap-10 border-b mb-6 max-md:px-4 border-grey pb-6 items-center">
                ...
                <div className="flex flex-col justify-between py-2 w-full min-w-[300px]">
                    <div>
                        ...
                        <p className="line-clamp">Published on { getDay(publishedAt) }</p>
                    </div>

                    <div className="flex gap-6 mt-3">
                        <Link to={`/editor/${blog_id}`} className="pr-4 py-2 underline tag over:bg-black-100 hover:opacity-75 px-4 py-2">Edit</Link>  

                        <button className=" lg:hidden pr-4 py-2 underline  tag hover:bg-black-100 hover:opacity-75 px-4 py-2" onClick={() => setShowStat(preVal => !preVal)}>Stats</button>
                        <button className="pr-4 py-2 underline text-red tag hover:bg-red-100 hover:opacity-75 px-4 py-2" onClick={(e) => deleteBlog(blog, access_token, e.target)}>Delete</button>
                    </div>
                </div>
