...
import admin from 'firebase-admin'; // using server side to access parent
import serviceAccountKey from "./contents-hub-9375d-firebase-adminsdk-uwoij-c66b225f5c.json" assert { type: "json" } ; // imp coz to make node ware that its a json file
...
server.use(express.json()); //enables json sharing and recieving of data
server.use(cors()); // allows cross resource origin i.e access to other ports too in this case both front end and backend are on differrent ports (enables server to accept data from any where)
...
const generateUsername = async(email) => {
    ...
    const isUsernameExist = await User.exists({"personal_info.username": username}).then( (result) => result);
...
server.post('/signup',  (req,res) => {
    ...
    bcrypt.hash(password, 10, async (err, hashed_password) => { // 10 represents the salting i.e no of times the hashing process repeated to make the password more complex
        
        const  username = await generateUsername(email);// eg as@gmail then [as,gmail] and username will store as
...
server.post("/google-auth", async(req, res) => {
    ...
    .then(async (decodedUser) => {
        ...
        picture = picture.replace("s96-c","s384-c"); //changing the resolution of stored image

        //adding uer to the database
        //fetching document
        ...
        if(user) {//login
            ...
            user = new User({
                personal_info: {fullname: name, email, profile_img: picture, username}, // remove the img if not showing default by google, it will take some random img 
                google_auth:true
            })
   ...
   .then((user) => {
    ...
    bcrypt.compare(currentPassword, user.personal_info.password, (err, result) => {
        ...
        bcrypt.hash(newPassword, 10 , (err, hashed_password) => {
            ...
            .then((u) => {
                return res.status(200).json({ status: 'pasword changed'})
            })
            .catch(err => {
                return res.status(200).json({ error: 'Some error occurred while saving new password, please try again later'})
...
server.get("/trending-blogs", (req,res) => {
    ...
    Blog.find({draft: false})
    .populate("author", "personal_info.profile_img  personal_info.username personal_info.fullname -_id")
    .sort("blog_id title publishedat -_id")
    .limit(5)
    .then(blogs => {
        return res.status(200).json({ blogs });
    })
...
// this is for applying load more func to srch by category
...
server.post("/update-profile", verifyJWT, (req,res) => {
    ...
    User.findOneAndUpdate({ _id: req.user }, updateObj, {
        runValidators: true // to make sure that username is unique
    })
    .then(() => {
...
server.post("/isLiked-by-users",verifyJWT, (req,res) =>{
...
server.post("/add-comment", verifyJWT, (req, res) =>{
    ...
    new Comment(commentObj).save().then(async (commentFile) => {
        ...
        Blog.findOneAndUpdate({ _id }, { $push: { "comments": commentFile._id },   $inc: {
            ...
            new Notification(notificationObj).save().then(notificaion => console.log('new notification created'));
...
// getting comments data  #blog-comment-container - fetched
...
const deleteComments = (_id) => {
    ...
    .then(comment => {
        ...
        Notification.findOneAndDelete({ comment: _id}).then(notificaion => console.log('comment notification deleted'));
...
server.post('/notifications', verifyJWT, (req, res) =>{
    ...
    if(deletedDocCount){
        skipDocs -= deletedDocCount; // Subtract the number of docs that were
    }
